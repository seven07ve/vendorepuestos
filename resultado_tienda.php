...
/*if(isset($_POST["palabra_tienda"]))$palabra_tienda=$_POST["palabra_tienda"];
if(isset($_GET["palabra_tienda"]))$palabra_tienda=$_GET["palabra_tienda"];*/
if(isset($_POST["palabra_tienda"]))
{
...
$trozos=explode(" ",$palabra_tienda);
  	...
  	if ($numero==1)
	{
		$_pagi_sql="SELECT p. * FROM productos p LEFT JOIN categoria c ON p.id_categoria = c.id WHERE (usuario_tienda='2' && id_usuario_tienda='$id' && p.vence>= NOW()) && (c.nombre LIKE '%$palabra_tienda%' OR p.descripcion LIKE '%$palabra_tienda%' OR p.titulo LIKE '%$palabra_tienda%' OR p.subtitulo LIKE '%$palabra_tienda%' OR p.id='$palabra_tienda')";
		if($_GET["p"]!=0) $_pagi_sql.=" ORDER BY precio ASC";
		if($_GET["f"]!=0) $_pagi_sql.=" ORDER BY vence ASC";
	}
	else
	{
		$_pagi_sql="SELECT p. *, MATCH(p.titulo,p.subtitulo,p.descripcion) AGAINST ('$palabra_tienda') AS puntuacion FROM productos p LEFT JOIN categoria c ON p.id_categoria = c.id WHERE (usuario_tienda='2' && id_usuario_tienda='$id' && p.vence>= NOW()) AND MATCH(p.titulo,p.subtitulo,p.descripcion) AGAINST ('$palabra_tienda') ";
		if($_GET["p"]!=0) $_pagi_sql.=" ORDER BY precio ASC, puntuacion DESC";
		if($_GET["f"]!=0) $_pagi_sql.=" ORDER BY vence ASC, puntuacion DESC";
	}
...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td valign="top">
<div class="titulo_ruta" style="height:30px; padding-top:10px;"><a href="/inicio/" class="titulo_ruta">vendorepuestos.com.ve</a> > <a href="/tr/<?=limpiar_cadena($vt["nombre_oficial"])?>/<?=$id?>/" class="titulo_ruta"><?=$vt["nombre_oficial"]?></a> > Resultados b&uacute;squeda: <b><?=$palabra_tienda;?></b></div>
</td>
...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <tr height="25" background="/imagenes/bg_botonera.jpg" class="menu">
  	...
  	<td width="110"><form action="/tr_resultado/<?=$carpeta?>/<?=$id?>/<?=$_GET["xx"];?>/1/0/1" method="post">PRECIO <input type="image" name="imageField" id="imageField" src="/imagenes/icon_arrow_sist_ne.jpg" /></form></td>
    <td width="110"><form action="/tr_resultado/<?=$carpeta?>/<?=$id?>/<?=$_GET["xx"];?>/0/1/1" method="post">
    VENCE 
...
th="30" align="right"><input name="input" type="image" src="/imagenes/btn_ir.jpg" /></td>
    </tr>
  	</form>
</table></td>
  </tr>
