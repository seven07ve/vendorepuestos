...
<link rel="stylesheet" type="text/css" href="/lib/skin_home_news.css" />
<script type="text/javascript">
jQuery(document).ready(function() {
    jQuery('#mycarousel-noticias').jcarousel();
});
</script>
</head>
...
<td width="300" valign="top"><div class="titcat"><div style="float:left; width:90px; height:20px;" align="right"><img src="/imagenes/icon_tr.jpg" width="28" height="20" border="0" /></div><div style="width:140px; height:20px; float:left">TIENDAREPUESTOS</div></div>
      ...
      <ul id="mycarousel-noticias" class="jcarousel-skin-tango-noticias">
      <? 
	  $noticia = mysql_query("SELECT * FROM noticias WHERE sumario!=''");
	  while($not = mysql_fetch_array($noticia))
	  {?>
<li>
            <div style="height:250px;"><img src="/imagenes_noticias/<?=$not["foto"]?>" width="255" /><br />
            <a href="<?=$not["link"]?>" target="_blank" class="gris"><?=$not["sumario"];?></a></div>
            <div style="height:25px; text-align:right; padding:5px 0 0 0"><a href="<? if($not["link"]!=""){ echo $not["link"]; } else { echo "/noticia/".limpiar_cadena($not["titulo"])."/1"; }?>"><img src="/imagenes/btn_vermas.png" width="72" height="26" border="0" /></a></div>
      </li>
        <?php }?>
      </ul>
     </td>
